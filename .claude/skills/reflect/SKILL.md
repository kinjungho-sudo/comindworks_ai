...
- 코마인드웍스 맥락: n8n 워크플로우 변경, Notion 업데이트, Jarvis 기능 추가 등 포함
...
Launch all 4 agents in a single response — running them sequentially would take 4x longer.
...
prompt: |
  ...
  Your job: Review existing documentation files (README.md, AGENTS.md, any .md files
  ...
  2. Read the most relevant ones (README.md, AGENTS.md first, then docs/)
...
prompt: |
  ...
  as a skill, script, or n8n workflow.

  Context: This is a 코마인드웍스 AI automation project. The stack includes:
  - n8n (workflow engine on Mac Mini)
  - Notion (knowledge base)
  - 텔레그램 (Jarvis interface)
  ...
  3. Consider: could this be an n8n workflow? A Claude Code skill? A Notion template?

  Return ONLY a structured list:
  ## automation-scout results
  - Pattern: [what was repeated] | Type: [skill/n8n-workflow/script/hook] | Suggestion: [how to automate]
...
prompt: |
  ...
  3. Note connections to existing knowledge (especially re: AI agents, n8n, automation)
...
prompt: |
  ...
  Context: 코마인드웍스 운영 맥락 — 
  - AI 자동화 에이전트 구축/교육 사업
  - Jarvis (총괄 오케스트레이터) 지속 개선
  - Foal AI 플랫폼 개발
  - n8n 기반 자동화 파이프라인 확장
  ...
  3. Prioritize by business impact (사업 임팩트 기준)
...
Then launch ONE more agent. **Before submitting, replace `{PHASE_1_RESULTS}` with actual output text.**
...
Present the duplicate-checked results clearly, then build a dynamic options list:
...
Then call `AskUserQuestion` with the relevant options and `multiSelect: true`.

Execute only the selected actions:

### 문서 업데이트
Use the `Edit` tool to apply the doc-updater's suggestions.
- Classify each change as `APPEND`, `REVISE`, or `NEW_SECTION`
- Do NOT rewrite unrelated sections
- Tell the user what was updated and in which file

### 자동화 생성
For each automation suggestion:
1. **skill** → use `skill-creator` (required); if not installed, tell the user
2. **n8n workflow** → describe the workflow spec for manual creation in n8n
3. **script** → write to `scripts/<name>.sh` or `scripts/<name>.py`
4. **hook** → add to `.claude/settings.json` under `hooks`

### 학습 기록 저장
Append learnings to `~/.claude/learnings.md` (create if it doesn't exist). Format:

```markdown
## YYYY-MM-DD — [project name / 코마인드웍스]
- [concept/tool/pattern]: [brief explanation]
- [concept/tool/pattern]: [brief explanation]
```

**Size check:** After appending, count `##` date sections. If more than 50, add:
```
...
- After completing a significant feature or milestone (Jarvis 기능 추가, n8n 워크플로우 완성 등)
